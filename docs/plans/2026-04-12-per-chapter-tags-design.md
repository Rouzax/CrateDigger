...
- **Stage-name filtering (audit finding):** the current scrape treats every `<meta itemprop="genre">` on the page as a genre, which incorrectly promotes stage names like `Mainstage` to the front of the list (41 / 79 existing files have `Mainstage` as their first genre). The audit suggests stage names appear in *page-header* genre metas, not per-track-row metas, so the new per-track aggregator should naturally demote them out of the top 5 without an explicit blocklist. Verify this when building the HTML parser (Task 9) by logging any genre that appears on more than N/2 tracks — if stage names are common per-track too, add a filter keyed off `source_cache` stage names.
- **Mojibake is rampant in the legacy corpus:** 52 / 79 enriched files have mojibake in chapter titles (`Ti├½sto`, `Am├⌐l`, `DI├ÿN`, `In├⌐z` — all from before commit `1e45b59`). Re-enrichment with the current fix applied will clean these up; this PR gains a sizable latent-bug-fix side benefit. The integration test (Task 17) must assert no mojibake survives.
- **`mkvmerge -J` tag inspection path:** tags appear under `global_tags` and `track_tags` in the JSON, not a single `tags` key. Task 12 and Task 17 assertions target those paths.
...
5. **Integration:** end-to-end re-enrichment of the Afrojack EDC 2025 MKV at `/home/user/_temp/cratedigger/data/mkv-info-dump/...`:
