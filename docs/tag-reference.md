...
| `CRATEDIGGER_TRACK_PERFORMER_SLUGS` | identify | Pipe-separated 1001Tracklists slugs for every artist on the track. For mashup chapters, all per-component artists are listed. | `afrojack\|oliver-heldens` |
| `CRATEDIGGER_TRACK_PERFORMER_NAMES` | identify | Pipe-separated display names for every artist, aligned with `CRATEDIGGER_TRACK_PERFORMER_SLUGS`. For mashup chapters, each component artist is listed individually. Artists sharing one `ft. A & B` credit each get their own name (not the combined text repeated). Acts 1001Tracklists registers as a single combined entity (e.g. an `A & B pres. C` alias with one slug) remain one combined name. | `Afrojack\|Oliver Heldens` |
...
| `CRATEDIGGER_TRACK_GENRE` | identify | Pipe-separated per-track genres. For mashup chapters, genres from all component tracks are aggregated and de-duplicated. | `Big Room\|Electro House` |
...
If KIKI's MBID is not yet in the cache, her slot in `CRATEDIGGER_ALBUMARTIST_MBIDS` and the per-chapter `MUSICBRAINZ_ARTISTIDS` will be an empty string. Add her to your `artist_mbids.json` (`~/CrateDigger/artist_mbids.json` on Linux and macOS, `Documents\CrateDigger\artist_mbids.json` on Windows) and re-run:
