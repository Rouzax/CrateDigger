...
| `CRATEDIGGER_TRACK_PERFORMER_NAMES` | identify | Pipe-separated display names for every artist, aligned with `CRATEDIGGER_TRACK_PERFORMER_SLUGS`. Mashup chapters carry the real per-component names instead of a single concatenated string. Artists sharing one `ft. A & B` credit each get their own name (not the combined text repeated). Acts 1001Tracklists registers as a single combined entity (e.g. an `A & B pres. C` alias with one slug) remain one combined name. | `Afrojack\|Oliver Heldens` |
