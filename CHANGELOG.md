...
- The "new version available" notice no longer crashes when output is piped on Windows. The notice used a Unicode arrow; when stdout is a pipe (scheduled tasks, log capture), Python encodes with the legacy codepage (cp1252) and the arrow raised UnicodeEncodeError, killing the command exactly when an update was available. The arrow is now ASCII ("0.34.1 -> 0.34.2"), and the CLI additionally reconfigures stdout/stderr at startup so any unencodable character anywhere in the output (exotic track titles included) degrades to "?" instead of crashing.
...
- The remaining false deprecation claims are gone from the documentation. `unknown_festival` and `festival_background_priority` were described as aliases that copy their value forward and log a one-shot warning; both were removed in 0.15.0, are read by nothing, and are silent no-ops, which the fallback-values, poster-settings, and Places pages now say. `festivals.json` was described as a file that gets read whenever `places.json` is missing; in reality startup copies it to `places.json` once, silently and only when no `places.json` is there yet (shared data folder only, originals left in place), and a library-local copy still needs a manual rename, so the migration guidance was rewritten around that and the invented 1.0.0 removal deadline dropped. The `.cratedigger/festivals/<name>/` logo fallback documented in the configuration, library-layout, and audit-logos pages does not exist either: curated logos are searched under `.cratedigger/places/<name>/` and then the shared `places/` folder, and edition logos live in a sibling folder named after the edition display name (`places/EDC Las Vegas/`), not a nested `<canonical>/<edition>/` path. Three links to the removed `festivals.md` page now point at `places.md`, so the docs build passes `--strict` again.
