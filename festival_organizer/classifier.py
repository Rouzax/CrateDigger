...
def classify(media_file: MediaFile, root: Path, config: Config) -> str:
    ...
    if config.is_forced_concert(rel):
        logger.info(
            "classifier.result: file=%s type=concert_film reason=config_forced",
            media_file.source_path.name,
        )
        return "concert_film"
    if config.is_forced_festival(rel):
        logger.info(
            "classifier.result: file=%s type=festival_set reason=config_forced",
            media_file.source_path.name,
        )
        return "festival_set"
    ...
    if media_file.metadata_source == "1001tracklists":
        logger.info(
            "classifier.result: file=%s type=festival_set reason=1001tracklists_metadata",
            media_file.source_path.name,
        )
        return "festival_set"
    ...
    if media_file.festival:
        ...
        if canonical in config.known_places:
            logger.info(
                "classifier.result: file=%s type=festival_set reason=known_festival festival=%s",
                media_file.source_path.name,
                canonical,
            )
            return "festival_set"
