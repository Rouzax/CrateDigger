...
def test_render_update_banner_only_when_behind():
    behind = _report(
        [_fest("A", "E", "2026", [], "")], update=UpdateInfo("0.19.9", "0.20.0", True)
    )
    current = _report(
        [_fest("A", "E", "2026", [], "")], update=UpdateInfo("0.20.0", "0.20.0", False)
    )
...
def test_render_caps_sets_and_shows_overflow():
    ...
    sets = [
        _fest(f"Artist{i}", "UMF Miami", "2026", [], "") for i in range(MAX_SETS + 5)
    ]
...
def test_render_poster_column_is_proportional_not_fixed():
    out = render(
        _report([_fest("A", "E", "2026", [], "19 tracks")]),
...
def test_render_row_repeats_event_on_card():
    ...
    out = render(
        _report([_fest("Eric Prydz", "UMF Miami", "2026", ["Techno"], "19 tracks")]),
        thumbs={},
    )
