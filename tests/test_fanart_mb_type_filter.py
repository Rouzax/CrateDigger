...
def test_mb_search_accepts_typeless_artist_entry():
    ...
    with patch(
        "festival_organizer.fanart.requests.get", return_value=_fake_get(200, response)
    ):
...
def test_mb_search_accepts_group_type():
    ...
    with patch(
        ...
        assert (
            _mb_search("Swedish House Mafia") == "9c9f1380-2516-4fc9-a3e6-f9f61941d12d"
        )
