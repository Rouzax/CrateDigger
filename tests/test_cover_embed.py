...
def test_ensure_cover_land_preserves_landscape_cover(tmp_path):
    ...
    with (
        patch.object(cover_embed, "extract_attachment", side_effect=fake_extract),
...
def test_ensure_cover_land_recovers_from_thumb_when_no_landscape(tmp_path):
    ...
    def fake_extract(src, att_id, dest):
        _img(
            Path(dest), (1000, 1500)
        )  # the existing cover is portrait, not a landscape
        return True
...
def test_set_primary_cover_renames_landscape_png_slot(tmp_path):
    target = tmp_path / "v.mkv"
    target.touch()
    poster = tmp_path / "v-poster.jpg"
    _img(poster, (1000, 1500))
    atts = [{"id": 1, "file_name": "cover.png", "content_type": "image/png"}]
    with (
        patch.object(cover_embed, "replace_attachment", return_value=True) as rep,
...
def test_converge_calls_ensure_before_set(tmp_path):
    ...
    calls = []
    with (
        patch.object(cover_embed, "list_image_attachments", return_value=[]),
        patch.object(
            cover_embed,
            "ensure_cover_land",
            side_effect=lambda *a: calls.append("ensure") or True,
        ),
        patch.object(
            cover_embed,
            "set_primary_cover",
            side_effect=lambda *a: calls.append("set") or True,
        ),
...
def test_converge_aborts_when_landscape_not_preserved(tmp_path):
    ...
    with (
        ...
        ok = cover_embed.converge_cover_attachments(
            target, poster, tmp_path / "v-thumb.jpg"
        )
