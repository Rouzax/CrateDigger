...
def test_list_image_attachments_nonzero_returncode(tmp_path):
    ...
    src.touch()
    with patch("festival_organizer.metadata.MKVMERGE_PATH", "/usr/bin/mkvmerge"), \
         patch("festival_organizer.mkv_attachments.tracked_run", return_value=_proc("", rc=1)):
...
def test_write_helpers_build_correct_argv(tmp_path):
    ...
    with patch("festival_organizer.metadata.MKVPROPEDIT_PATH", "/usr/bin/mkvpropedit"), \
        ...
        assert att.replace_attachment(target, "cover.png", data, "cover.jpg", "image/jpeg") is True
        assert att.delete_attachment(target, "cover_land.png") is True

    assert calls[0] == ["/usr/bin/mkvpropedit", str(target),
                        "--attachment-name", "cover.jpg", "--attachment-mime-type", "image/jpeg",
                        "--add-attachment", str(data)]
    assert calls[1] == ["/usr/bin/mkvpropedit", str(target),
                        "--attachment-name", "cover.jpg", "--attachment-mime-type", "image/jpeg",
                        "--replace-attachment", f"name:cover.png:{data}"]
    assert calls[2] == ["/usr/bin/mkvpropedit", str(target), "--delete-attachment", "name:cover_land.png"]
