...
def test_find_by_type(tmp_path):
    ...
    cache.put(
        "c",
        {"name": "ADE", "slug": "ade", "type": "Conference", "country": "Netherlands"},
    )
...
def test_promote_event_promoter_to_festival(tmp_path):
    ...
    cache.put(
        "7xp1dkc",
        {
            "name": "Historic Virginia Key Park",
            "slug": "hvkp",
            "type": "Event Location",
            "country": "United States",
        },
...
def test_cache_load_logs_not_found(tmp_path, caplog):
    ...
    with caplog.at_level(
        logging.DEBUG, logger="festival_organizer.tracklists.source_cache"
    ):
...
def test_cache_load_logs_entry_count(tmp_path, caplog):
    ...
    assert any(
        "source_cache.load:" in msg and "entries=1" in msg for msg in caplog.messages
    )
