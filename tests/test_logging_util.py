...
def test_logger_records_actions():
    ...
    mf = MediaFile(
        source_path=Path("src.mkv"), artist="Test", festival="AMF", year="2024"
    )
    fa = FileAction(
        source=Path("src.mkv"), target=Path("dst.mkv"), media_file=mf, status="done"
    )
...
def test_logger_stats():
    ...
    logger.log_action(
        FileAction(source=Path("a"), target=Path("b"), media_file=mf, status="done")
    )
    logger.log_action(
        FileAction(
            source=Path("c"),
    ...
    logger.log_action(
        FileAction(source=Path("e"), target=Path("f"), media_file=mf, status="done")
    )
    stats = logger.stats
    assert stats["done"] == 2
